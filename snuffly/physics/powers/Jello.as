package snuffly.physics.powers
{
	...
	import snuffly.physics.particles.FluidParticle;
	import flash.utils.*;
	...
	{
		protected var pt:Vector.<FluidParticle>;				//Частицы желе
		...
		protected var activeParticles:Vector.<BaseParticle>;	//Активный кусок желе
		...
		protected function particlesChanged(event:ParticleGroupEvent):void
		{
			...
			pt			=Vector.<FluidParticle>(event.particles);
			ptCount		=pt.length;
			...
			activeParticles=new Vector.<BaseParticle>(ptCount,true);
		...
		public function applyPower():void
		{
			...
			if(activeChanged)
			{
				if(jelloState)
				{
					...
					activeParticles=new Vector.<BaseParticle>(ml,true);
					...
					activeParticles=Vector.<BaseParticle>(pt);
				notifyGroupChanged();
			}
		...
		{	/*
			...
			*/
			...
			var r1:Rectangle=new Rectangle(0, 0, 0, 0);
			for (i=0; i<ptCount; i++)
			{
				p=pt[i];
				r=p.r;
				pnt=new Point(p.xx-r,p.yy-r);
				r*=2;
				r1.width=r;
				r1.height=r;
				bmp.copyPixels(p.bubbleBmp,r1,pnt,null,null,true);
			}
